...
{
    public class DTravelArticle : DALBase, ITravelArticle
    {
        ...
        public bool Add(EyouSoft.Model.MTravelArticle model)
        {
            ...
            StrSql += ",TitleColor,KeyWords,ClassId,IsFrontPage,IsHot,IssueTime,OperatorId,LinkUrl,Click,SortRule,JiBie)";
            ...
            this._db.AddInParameter(dc, "JiBie", DbType.Int32, model.JiBie);
        ...
        public bool Update(EyouSoft.Model.MTravelArticle model)
        {
            ...
            StrSql += ",OperatorId=@OperatorId,LinkUrl=@LinkUrl,SortRule=@SortRule,JiBie=@JiBie ";
        ...
        public EyouSoft.Model.MTravelArticle GetModel(string ArticleID)
        {
            ...
            string StrSql = "SELECT ArticleID,Source,ArticleTitle,ImgPath,Description,ArticleText,ArticleTag,TitleColor,KeyWords,ClassId,(select top 1 ClassName from tbl_TravelArticleClass where ClassId=tbl_TravelArticle.ClassId) as ClassName,IsFrontPage,IsHot,IssueTime,OperatorId,(select RealName from tbl_Webmaster where Id=tbl_TravelArticle.OperatorId) as OperatorName,LinkUrl,Click,SortRule FROM tbl_TravelArticle WHERE ArticleID=@ArticleID";
            DbCommand dc = this._db.GetSqlStringCommand(StrSql.ToString());
            this._db.AddInParameter(dc, "ArticleID", DbType.AnsiStringFixedLength, ArticleID);
        ...
        public IList<EyouSoft.Model.MTravelArticle> GetList(int pageSize, int pageIndex, ref int recordCount, EyouSoft.Model.MTravelArticleCX chaXun, IList<EyouSoft.Model.TravelArticleOrderBy> FiledOrder)
        {
            ...
            fields.Append("(select top 1 ClassName,IsSystem from tbl_TravelArticleClass where ClassId=tbl_TravelArticle.ClassId for xml raw,root('Root')) as ClassName,(select RealName from tbl_Webmaster where Id=tbl_TravelArticle.OperatorId) as OperatorName");
            string query = " 1=1 ";
            if (chaXun != null)
            {
                ...
                if (chaXun.OperatorId > 0)
                {
                    query = query + string.Format(" AND OperatorId={0}", chaXun.OperatorId);
                }
                if (chaXun.IsSystem != null && chaXun.IsSystem.Length > 0)
                {
                    query = query + string.Format(" AND exists(select 1 from tbl_TravelArticleClass where ClassId=tbl_TravelArticle.ClassId and IsSystem in ({0})) ", Toolkit.Utils.GetSqlIn(chaXun.IsSystem));
                }
                if (!string.IsNullOrEmpty(chaXun.OperatorName))
                {

                    query = query + string.Format(" AND exists(select 1 from tbl_Webmaster where Id=tbl_TravelArticle.OperatorId and RealName like '%{0}%') ", chaXun.OperatorName);
                }
                if (chaXun.JiBie != null)
                {
                    query = query + string.Format(" AND JiBie<={0}", (int)chaXun.JiBie);
                }
            }
        ...
        {
            IList<EyouSoft.Model.MTravelArticle> ResultList = null;
            string StrSql = string.Format("SELECT {0} ArticleID,Source,ArticleTitle,ImgPath,Description,ArticleText,ArticleTag,TitleColor,KeyWords,ClassId,(select top 1 ClassName,IsSystem from tbl_TravelArticleClass where ClassId=tbl_TravelArticle.ClassId for xml raw,root('Root')) as ClassName,IsFrontPage,IsHot,IssueTime,OperatorId,(select RealName from tbl_Webmaster where Id=tbl_TravelArticle.OperatorId) as OperatorName,LinkUrl,Click,SortRule FROM tbl_TravelArticle WHERE 1=1 ", (Top > 0 ? " TOP " + Top + " " : ""));
            if (chaXun != null)
            {
                if (!string.IsNullOrEmpty(chaXun.Source))
                {
                ...
                if (chaXun.IsSystem != null && chaXun.IsSystem.Length > 0)
                {
                    StrSql = StrSql + string.Format(" AND exists(select 1 from tbl_TravelArticleClass where ClassId=tbl_TravelArticle.ClassId and IsSystem in ({0})) ", Toolkit.Utils.GetSqlIn(chaXun.IsSystem));
                }
            }
            string orderByString = "";
            if (FiledOrder != null && FiledOrder.Count > 0)
        ...
        /// <returns></returns>
        public IList<EyouSoft.Model.MTravelArticle> GetTopList(int Top, int classid)
        {
            IList<EyouSoft.Model.MTravelArticle> ResultList = null;
            string StrSql = string.Format("SELECT top {0} ArticleID,ArticleTitle FROM tbl_TravelArticle where  ClassId={1} and IsFrontPage=1 ", Top, classid);
            StrSql = StrSql + " ORDER BY IssueTime desc";
            DbCommand dc = this._db.GetSqlStringCommand(StrSql.ToString());
            using (IDataReader dr = EyouSoft.Toolkit.DAL.DbHelper.ExecuteReader(dc, this._db))
            {
                ResultList = new List<EyouSoft.Model.MTravelArticle>();
        ...
        public IList<Model.MTravelArticle> GetHeadZiXun()
        {
            ...
            //strSql.AppendFormat(
            //    " SELECT top 3 ArticleID,Source,ArticleTitle,ImgPath,Description,ArticleText,ArticleTag,TitleColor,KeyWords,ClassId,(select top 1 ClassName,IsSystem from tbl_TravelArticleClass where ClassId=tbl_TravelArticle.ClassId for xml raw,root('Root')) as ClassName,IsFrontPage,IsHot,IssueTime,OperatorId,(select RealName from tbl_Webmaster where Id=tbl_TravelArticle.OperatorId) as OperatorName,LinkUrl,Click,SortRule FROM tbl_TravelArticle WHERE exists(select 1 from tbl_TravelArticleClass where ClassId = tbl_TravelArticle.ClassId and IsSystem = {0}) order by SortRule DESC,IssueTime DESC ; ",
            //    (int)Model.Enum.ArticleType.安全指南);
            //strSql.AppendFormat(
            //    " SELECT top 3 ArticleID,Source,ArticleTitle,ImgPath,Description,ArticleText,ArticleTag,TitleColor,KeyWords,ClassId,(select top 1 ClassName,IsSystem from tbl_TravelArticleClass where ClassId=tbl_TravelArticle.ClassId for xml raw,root('Root')) as ClassName,IsFrontPage,IsHot,IssueTime,OperatorId,(select RealName from tbl_Webmaster where Id=tbl_TravelArticle.OperatorId) as OperatorName,LinkUrl,Click,SortRule FROM tbl_TravelArticle WHERE exists(select 1 from tbl_TravelArticleClass where ClassId = tbl_TravelArticle.ClassId and IsSystem = {0} ) order by SortRule DESC,IssueTime DESC ; ",
            //    (int)Model.Enum.ArticleType.订购指南);
            //strSql.AppendFormat(
            //    " SELECT top 3 ArticleID,Source,ArticleTitle,ImgPath,Description,ArticleText,ArticleTag,TitleColor,KeyWords,ClassId,(select top 1 ClassName,IsSystem from tbl_TravelArticleClass where ClassId=tbl_TravelArticle.ClassId for xml raw,root('Root')) as ClassName,IsFrontPage,IsHot,IssueTime,OperatorId,(select RealName from tbl_Webmaster where Id=tbl_TravelArticle.OperatorId) as OperatorName,LinkUrl,Click,SortRule FROM tbl_TravelArticle WHERE exists(select 1 from tbl_TravelArticleClass where ClassId = tbl_TravelArticle.ClassId and IsSystem = {0} ) order by SortRule DESC,IssueTime DESC ; ",
            //    (int)Model.Enum.ArticleType.沟通与订阅);
            //strSql.AppendFormat(
            //    " SELECT top 3 ArticleID,Source,ArticleTitle,ImgPath,Description,ArticleText,ArticleTag,TitleColor,KeyWords,ClassId,(select top 1 ClassName,IsSystem from tbl_TravelArticleClass where ClassId=tbl_TravelArticle.ClassId for xml raw,root('Root')) as ClassName,IsFrontPage,IsHot,IssueTime,OperatorId,(select RealName from tbl_Webmaster where Id=tbl_TravelArticle.OperatorId) as OperatorName,LinkUrl,Click,SortRule FROM tbl_TravelArticle WHERE exists(select 1 from tbl_TravelArticleClass where ClassId = tbl_TravelArticle.ClassId and IsSystem = {0} ) order by SortRule DESC,IssueTime DESC ; ",
            //    (int)Model.Enum.ArticleType.支付与取票);
        ...
        private void GetClassBySqlXml(string xml, ref Model.MTravelArticle model)
        {
            ...
            foreach (var t in xRows)
            {
                ...
                model.ArticleType = (Model.Enum.ArticleType)Utils.GetInt(Utils.GetXAttributeValue(t, "IsSystem"));
        ...
        public EyouSoft.Model.MTravelArticleLY GetLiuYanModel(string LiuYanId)
        {
            ...
            strSql.Append(" (select top 1 Username from tbl_Webmaster where Id=tbl_TravelArticleLY.OperatorId ) as Username ");
        ...
        public IList<EyouSoft.Model.MTravelArticleLY> GetLiuYanList(int PageSize, int PageIndex, ref int RecordCount, EyouSoft.Model.MTravelArticleLYCX chaxun)
        {
            ...
            fields.Append(" (select top 1 Username from tbl_Webmaster where Id=tbl_TravelArticleLY.OperatorId ) as Username ");
