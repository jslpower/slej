...
.dindan_mx td{ font-size:12px;}
    ...
    <div class="warp">
        ...
        <div class="mt10 user_T font16">酒店价格列表</div>
     <div class="dindan_mx">
         <table width="100%" border="0" cellpadding="0" cellspacing="0">
              <tr>
                <th>日期</th>
             ...
             <asp:Repeater ID="JiageList" runat="server">
             <ItemTemplate>
             <tr>
                <td align="center"><%# Eval("ChenkInDate","{0:yyyy-MM-dd}")%></td>
                 <td align="center"><span class="font_yellow"><%# Eval("MenShiJia", "{0:f2}")%></span>元/间/天*<%= RoomNum%>间=<span class="font_red"><%# (Convert.ToDecimal(Eval("MenShiJia"))*RoomNum).ToString("f2")%></span>元/天</td>
                <td align="center"><%# Convert.ToDecimal(Eval("ChengBenJia").ToString()) == 0 ? "总站交易" : Eval("ChengBenJia", "{0:f2}") + "元/间/天*" + RoomNum + "间=<span class=\"font_red\">" + (Convert.ToDecimal(Eval("ChengBenJia")) * RoomNum).ToString("f2") + "</span>元/天"%></td>
              </tr>
             </ItemTemplate>
             </asp:Repeater>
         </table>
