...
{
    public partial class TuanGouOrderXX : HuiYuanWapPageBase
    {
        ...
        {
            WapHeader1.HeadText = "促销秒杀订单";
            initPage();
        }
        void initPage()
        {
            var model = new EyouSoft.BLL.OtherStructure.BTuanGouDingDan().GetModel(Utils.GetInt(Utils.GetQueryStringValue("orderid")));
            if (model != null)
            {
                ltrChanPinName.Text = model.ProductName;
                ltrOrderCode.Text = model.OrderCode;
                txtContact.Text = model.PeopleName;
                litInTime.Text = model.IssueTime.ToString();
                litJinE.Text = model.OrderPrice.ToString("C2");
                txtContactTel.Text = model.PeopleMobile;
                ...
                if (model.Peopleaddress != null) ltrDiZhi.Text = model.Peopleaddress;

                AnNiu.Text = getZhiFuURL(model.OrderID.ToString(), model.OrderState, model.SupplierID, model.PeopleID);
            }
        }
        /// <summary>
        /// 获取支付链接
        ...
        protected string getZhiFuURL(string orderid, object state, object AgencyId, object xdrid)
        {
            ...
            switch (type)
            {
                ...
                case EyouSoft.Model.Enum.XianLuStructure.OrderStatus.待付款:
                    return string.Format("<a class=\"y_btn\" href='javascript:;' onclick=\"javascript:pageData.Pay('{0}','{1}','{2}');\"  >马上付款</a>", orderid, (int)DingDanType.团购订单, HuiYuanInfo.UserId);
