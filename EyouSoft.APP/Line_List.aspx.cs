...
namespace EyouSoft.WAP
{
    public partial class Line_List : Common.Page.WebPageBase
    {
        ...
        void initList()
        {
            ...
            {
                #region 设置微信分享链接
                WapHeader1.FenXiangBiaoTi = !string.IsNullOrEmpty(_wxTitle) ? _wxTitle : WapHeader1.HeadText;
                WapHeader1.FenXiangMiaoShu = Utils.InputText(list[0].Description);
                WapHeader1.FenXiangTuPianFilepath = "http://" + Request.Url.Host + TuPian.F1(list[0].TeSeFilePath, 210, 70, list[0].XianLuId);
                #endregion
