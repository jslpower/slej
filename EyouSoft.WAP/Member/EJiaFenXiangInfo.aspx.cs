...
namespace EyouSoft.WAP.Member
{
    public partial class EJiaFenXiangInfo : WebPageBase
    {
            ...
            if (model != null)
            {
                ...
                if (model.YouJiContent.Any())
                {
                    ...
                    if (!string.IsNullOrEmpty(model.YouJiContent[0].ImgFile))
                    {
                        ...
                        FenXiangTuPianFilepath = "http://" + Request.Url.Host + TuPian.F1(model.YouJiContent[0].ImgFile, 640, 400, model.YouJiId);
                    }
                    else if (!string.IsNullOrEmpty(setimg))
                    {
                        FenXiangTuPianFilepath = "http://" + Request.Url.Host + TuPian.F1(setimg, 640, 400, model.YouJiId);
                    }
                    else
                    {
                        FenXiangTuPianFilepath = "http://" + Request.Url.Host + "/images/NoPic.jpg";
                    }
                }
                FenXiangBiaoTi = model.YouJiTitle;
        ...
        protected string getImgSrc(object objStr, string youjiid)
        {
            ...
            return string.Format("<img src=\"{0}\">", TuPian.F1(src, 320, 240, youjiid));
        }
