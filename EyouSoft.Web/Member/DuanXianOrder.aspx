<%@ Page Title="短线订单列表" Language="C#" MasterPageFile="~/MasterPage/Front2.Master"
    AutoEventWireup="true" CodeBehind="DuanXianOrder.aspx.cs" Inherits="EyouSoft.Web.Member.DuanXianOrder" %>
        ...
        <div class="user_dindan">
            ...
            <table width="100%" border="0" class="tableList margin_T10">
                   <tr>
                     ...
                     <th width="66" align="center">发团时间</th>
                     <th width="130" align="center">交易金额</th>
                     <%if (isAgency == 1)
                       { %>
                     <th width="130" align="center">分销金额</th>
                     <th width="70" align="center">分销利润</th>
                     <%} %>
                     <th width="80" align="center">客人信息</th>
              ...
              <ItemTemplate>
                   <tr>
                     <td height="28" align="center"><a href="XianLuDetail.aspx?orderid=<%# Eval("OrderId")%>"><%# Eval("OrderCode")%></a></td>
                     <td align="left"><a href="/print/xingchengdan.aspx?routeid=<%# Eval("XianLuId")%>" target="_blank" class="fontblue" title="<%# EyouSoft.Common.Utils.ConverToStringByHtml(Eval("XianLuName").ToString())%>"><%# EyouSoft.Common.Utils.GetText2(EyouSoft.Common.Utils.ConverToStringByHtml(Eval("XianLuName").ToString()), 24, true)%></a></td>
                     <td align="center"><%# Convert.ToDateTime(Eval("LDate")).ToString("yyyy-MM-dd")%></td>
                     <td align="left">
                     人数：<%# Eval("ChengRenShu")%>成人<%# Eval("ErTongShu")%>儿童<br />
                     成人价：<%# UtilsCommons.GetGYStijia(EyouSoft.Model.Enum.FeeTypes.周边线路, Convert.ToDecimal(Eval("JSJCR")), Convert.ToDecimal(Eval("SCJCR")), (EyouSoft.Model.Enum.MemberTypes)EyouSoft.Common.Utils.GetInt(((int)Eval("UserType")).ToString())).ToString("f2")%> 元/人<br />
                     儿童价：<%# UtilsCommons.GetGYStijia(EyouSoft.Model.Enum.FeeTypes.周边线路,Convert.ToDecimal(Eval("JSJER")),Convert.ToDecimal(Eval("SCJET")), (EyouSoft.Model.Enum.MemberTypes)EyouSoft.Common.Utils.GetInt(((int)Eval("UserType")).ToString())).ToString("f2") %> 元/人<br />
                     金额：<%#  Convert.ToDouble(Eval("JinE")).ToString("f2")%> 元</td>
                     <%if (isAgency == 1)
                     { %>
                     <td align="left">
                     人数：<%# Eval("ChengRenShu")%>成人<%# Eval("ErTongShu")%>儿童<br />
                     成人价：<%# UtilsCommons.GetGYStijia(EyouSoft.Model.Enum.FeeTypes.周边线路, Convert.ToDecimal(Eval("JSJCR")), Convert.ToDecimal(Eval("SCJCR")), (EyouSoft.Model.Enum.MemberTypes)EyouSoft.Common.Utils.GetInt(((int)HuiYuanInfo.UserType).ToString())).ToString("f2")%> 元/人<br />
                     儿童价：<%# UtilsCommons.GetGYStijia(EyouSoft.Model.Enum.FeeTypes.周边线路, Convert.ToDecimal(Eval("JSJER")), Convert.ToDecimal(Eval("SCJET")), (EyouSoft.Model.Enum.MemberTypes)EyouSoft.Common.Utils.GetInt(((int)HuiYuanInfo.UserType).ToString())).ToString("f2")%> 元/人<br />
                     ...
                     <td align="left"><%# Eval("UserType")%>：<%# Eval("MemberName")%> <br />
                     <%# Eval("Mobile")%></td>
...
<asp:Content ID="Content3" ContentPlaceHolderID="Adv" runat="server">
    ...
    <script type="text/javascript">
     var pageData = {
         ...
         window.location.href="/Member/PayMoney.aspx?Pay=1&Classid=9&id=" + id + "&type=" + type + "&token=" + token;
//                Boxy.iframeDialog({
...
//                });
            ...
            setOrder: function(oid, state) {
                if (window.confirm("请确认操作")) {
                    $.ajax({
                        url: "/Member/DuanXianOrder.aspx?setState=1&id=" + oid + "&state=" + state,
                        dataType: "json",
