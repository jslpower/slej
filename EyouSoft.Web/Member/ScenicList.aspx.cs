...
{
    public partial class ScenicList : ClientModelViewPageBase<EyouSoft.Model.ScenicStructure.WebModel.MScenicAreaOrderSearchModel>
    {
        ...
        public void GetOrderList(string AgencyId)
        {
            ...
            if (scenicName.Value.Trim() != "")
            {
                Model.ScenicName = scenicName.Value.Trim();
            }
            if (!string.IsNullOrEmpty(starttime.Value.Trim()) && !string.IsNullOrEmpty(endtime.Value.Trim()))
            {
                ...
                {
                    Model.BeginTime = Convert.ToDateTime(starttime.Value.Trim());
            ...
            var list = bll.GetScenicAreaOrderList(10, PageIndex, ref TotalCount, Model);
        ...
        //            return string.Format("<a href='javascript:;' onclick=\"javascript:pageData.Pay('{0}','{1}','{2}');\"  >付款</a>", orderid, (int)EyouSoft.Model.Enum.DingDanLeiBie.门票订单, HuiYuanInfo.UserId);
        ...
        protected string getZhiFuURL(string orderid, object state, object AgencyId)
        {
            ...
            switch (type)
            {
                ...
                case EyouSoft.Model.Enum.XianLuStructure.OrderStatus.待付款:
                    return string.Format("<a href='javascript:;' onclick=\"javascript:pageData.Pay('{0}','{1}','{2}');\"  ><span style='background-color:#906;color:#FFF;'>马上付款</span></a>", orderid, (int)EyouSoft.Model.Enum.DingDanLeiBie.门票订单, HuiYuanInfo.UserId);
                case EyouSoft.Model.Enum.XianLuStructure.OrderStatus.订单出货:
