...
{
    public partial class TravelArticleList : EyouSoft.Common.Page.WebmasterPageBase
    {
        #region 分页参数
        ...
        protected string BindArticleClass(string selectItem)
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                //if (selectItem != "")
                //{
                if ((list[i].ClassId + 1).ToString() == selectItem && selectItem != "0")
                    {
                        query.AppendFormat("<option value='{0}' selected='selected'>{1}</option>", list[i].ClassId+1, list[i].ClassName);
                    }
                    else
                    {
                        query.AppendFormat("<option value='{0}' >{1}</option>", list[i].ClassId+1, list[i].ClassName);

                    }
                //}
                //else 
                //{
                //    if (list[i].ClassId.ToString() =="0")
                //    {
