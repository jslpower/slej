...
{
    public partial class ShangChengLianMeng : Common.Page.WebPageBase
    {
        #region 分页参数
        ...
        {
            int itemCount = 0;
            IList<MLianMengLeiBie> list = new EyouSoft.BLL.MallStructure.BLianMeng().GetList(1000, 1, ref itemCount, new MLianMengLeiBieSer() { modelTp = EyouSoft.Model.Enum.ModelTypes.购物广场联盟 });
            if (list != null && list.Count > 0)
            {
        ...
        private void InitAd()
        {
            ...
            //首页轮播图
            var top = bll.GetList(5, AdvArea.购物广场联盟);
